...
function groupByMonth(meetings: Meeting[], dfLocale: Locale): MonthGroup[] {
  const groups = new Map<string, MonthGroup>();
  for (const m of meetings) {
    const d = parseISO(m.createdTime);
    const key = format(d, 'yyyy-MM');
    const label = format(d, 'MMMM yyyy', { locale: dfLocale }).toUpperCase();
...
export function HistoryPanel({ meetings, currentId }: Props) {
  ...
  const groups = groupByMonth(meetings, dfLocale);
  ...
  return (
    <aside className="lg:border-l border-border bg-[#f7f7f8] overflow-auto h-full">
          ...
          {g.meetings.map((m) => {
            ...
            return (
              ...
              >
                ...
                >
                  {format(d, 'MMM d', { locale: dfLocale })} · {t('history.weekShort', { week })}
                </div>
                <div
                  className={cn(
      ...
      {groups.length === 0 && (
        <div className="text-[12px] text-muted-foreground text-center p-4">{t('history.empty')}</div>
      )}
    </aside>
  );
}
